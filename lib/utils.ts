...
// Configure assistant with voice and style
export const configureAssistant = (voice: string, style: string): CreateAssistantDTO => {
  const voiceOptions = voices[voice as keyof typeof voices];
  const voiceId = voiceOptions?.[style as keyof typeof voiceOptions] || "sarah";
